...
module.exports = class FixedLengthEncoder {
  ...
  isValidAlphabet() {
    ...
    var uniqueArray = alphabetArray.filter((elem, pos) => {
      return alphabetArray.indexOf(elem) == pos;
    });
    return alphabetArray.length == uniqueArray.length;
  }

  isValidMap() {
    var length = Math.pow(this.ALPHABET.length, 2);
    var valid = (this.ENCODE_MAP.length == length) && (this.DECODE_MAP.length == length);
    for (var index = 0; index < length; index++) {
      if (valid) valid = (this.DECODE_MAP[this.ENCODE_MAP[index]] == index);
    }
    return valid;
  }
}
...
class Encoder {
  constructor(alphabet, encodeMap, decodeMap) {
  ...
  encode(value, messageLength) {
    var value, string;
    if (!Number.isInteger(value)) throw('Cannot encode a non-integer');
    if (value < 0) throw('Cannot encode negative values');
    this.setup(messageLength, +1);
    if (value >= this.maxValue) throw(`Cannot encode ${value} in ${this.messageLength} characters`);
    console.log('+', value);
    value = this.offsetValue(value);
    console.log(' ', value);
    value = this.scrambleValue(value);
    console.log(' ', value);
    string = this.integerToMessage(value);
    console.log(' ', value);
    return string;
  }
  ...
  decode(message) {
    var value;
    if (typeof message != 'string') throw('Cannot decode a non-string.');
    this.setup(message.length, -1);
    console.log('!', message);
    value = this.messageToInteger(message);
    console.log(' ', value);
    value = this.scrambleValue(value);
    console.log(' ', value);
    value = this.offsetValue(value);
  ...
  setup(messageLength, direction) {
    this.messageLength = messageLength;
    this.maxValue = Math.pow(this.alphabet.length, messageLength);
    this.direction = direction;
  }

  offsetValue(value) {
    var offset = Math.floor(this.maxValue / 2);
    value = value + (this.direction * offset);
  ...
  scrambleValue(value) {
    var digitsArray = this.integerToDigitsArray(value);
    digitsArray = this.remapDigits(digitsArray);
    var newValue = this.digitsArrayToInteger(digitsArray);
    return newValue;
  }

  remapDigits(digitsArray) {
    var indexes = this.getCircularIndexes();
    indexes.forEach((cV, index) => {
      var lowIndex = indexes[index];
      var highIndex = indexes[index + 1];
      var mapIndex = digitsArray[highIndex] * this.base + digitsArray[lowIndex];
      var mapValue;
      if (this.direction > 0) mapValue = this.encodeMap[mapIndex];
      if (this.direction < 0) mapValue = this.decodeMap[mapIndex];
      digitsArray[lowIndex] = Math.floor(mapValue / this.base);
      digitsArray[highIndex] = mapValue % this.base;
    });
    return digitsArray;
  }
  ...
  getCircularIndexes() {
    ...
    indexes = Array(messageLength + 1).fill().map((x, i) => i % messageLength);
    if (this.direction < 0) indexes = indexes.reverse();
    return indexes;
  }

  integerToMessage(value) {
    var digitsArray = this.integerToDigitsArray(value);
    var message = digitsArray.map((value) => this.alphabet[value]).join('');
    return message;
  }

  integerToDigitsArray(value) {
    var message = [];
    var remainder;
    while (value > 0 || message.length < this.messageLength) {
  ...
  messageToInteger(message) {
    var digitsArray = message.split('').map(character => {
      var digit = this.alphabet.indexOf(character);
      if (digit == -1) throw(`Cannot decode an invalid character (${character})`);
      return digit;
    });
    var value = this.digitsArrayToInteger(digitsArray);
    return value;
  }

  digitsArrayToInteger(digitsArray) {
    var value = digitsArray.reduceRight((digit, value) => (value * this.base + digit), 0);
    return value;
  }
}
